...
# kayla altendorf
# major sections of code for specific analyses are commented out #### with four hashes and can be navigated easily in R Studio
...
setwd("/Users/garett/Desktop/HopData")

# read in data
dat <- read.csv("./Output/hopbox_data.csv")

#### run t-tests between subsamples and full samples #### 
# skip this if you want to use the data already generated (see below) 

# create a new column for genotype_rep
...
# read in the already produced output here
output_all <- read.csv("./output_all_02.20.23.csv")

#### calculate frequency of non-representative sample #### 
# determine the number of samples that were significantly different
...
# extract out the backbone of the tests that were conducted
backbone <- output_all %>% 
  mutate(merge = paste(genotype, trait, sample_size, sep = "_")) %>% 
  dplyr::select(merge) %>%
  distinct()

# and merge the number of significant tests with the backbone
...
# conduct a segmented regression
...
# make a matrix that can be used in segmented for all 8 traits at once
...
slope(sig.seg)
# none of the slope estimates change much so assume a linear relationship

sig.lm1<- lm(frequency ~ as.numeric(sample_size) * trait, data = sig)
anova(sig.lm1) # trait does not influence slope and does not interact with sample size
...
ggplot(sig, aes(x = as.numeric(sample_size), y = frequency, color = trait))  + 
  ...
  scale_color_manual('Trait', 
                     ...
                     values = c("#009E73", "#CC79A7", "#E69F00", "#D55E00", "#F0E442", "#56B4E9", "#0072B2", "grey30")) +
  ...
  labs(y = "Proportion of Significant T-Tests", x = "Sample Size") 

#### assess spearman rank correlations between subsample and full sample ####
# skip this if you want to read in the already produced results (see below) 
...
spearman.fitted <- fitted(spearman.seg)
spearman.prediction <- data.frame(sample_size = subsampling_spearman$sample_size, 
                                  trait = subsampling_spearman$trait,
                                  rho = spearman.fitted)
...
ggplot(subsampling_spearman, aes(x = sample_size, y = rho, color = trait))  + 
  geom_point(size = 0.01, position = "jitter") + 
  theme_bw() +
  ...
  theme(axis.text.x = element_text(size = 25),
        ...
        legend.text = element_text(size = 12),
        legend.position = "bottom") + 
  labs(y = "Spearman Rank Coefficient", x = "Sample Size")
...
# slope and breakpoint coefficients
zero_wrong.seg$psi 
slope(zero_wrong.seg)

# fitting a regression line using the segmented model
zero_wrong.fitted <- fitted(zero_wrong.seg)
zero_wrong.prediction <- data.frame(sample_size = zero_wrong$sample_size, 
                                  trait = zero_wrong$trait,
...
ggplot(zero_wrong, aes(x = sample_size, y = frequency, color = trait))  + 
  ...
  geom_line(data=zero_wrong.prediction, 
            mapping=aes(x = sample_size, y = frequency, color = trait),
            size=1) +
  scale_color_manual('Trait',
                     labels = c("Area", "Density", "DGCI", "Length", "Openness", "Perimeter", "Weight", "Width"),
                     values = c("#009E73", "#CC79A7", "#E69F00", "#D55E00", "#F0E442", "#56B4E9", "#0072B2", "grey30")) +
  ...
  theme(axis.text.x = element_text(size = 25),
        ...
        legend.text = element_text(size = 12),
        legend.position = "bottom") + 
  labs(y = expression( bold("Probability ">="1 Erroneous Selections", )), x = "Sample Size") 
...
# fitting a regression line using the segmented model
f_val.fitted <- fitted(f_val.seg)
f_val.prediction <- data.frame(sample_size = f_val$sample_size, 
                                  trait = f_val$trait,
                               f_val = f_val.fitted)
